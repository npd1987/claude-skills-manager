 ...
 *
 * session.json is deliberately not carried across, because it is throwaway runtime
...
module.exports = {
  ...
  // Whether the update check may run, what it last found, and which versions
  // this install has been through. Kept here rather than in the user's
  // settings.json, which is theirs and not ours to write preferences into.
  PREFS_FILE: path.join(DATA_DIR, 'prefs.json'),
  // Written by the detached updater after the app has exited, and read on the
  // next launch to report how it went.
  APPLY_RESULT_FILE: path.join(DATA_DIR, 'last-update.json'),
  SESSIONS_DIR,
