...
// The prefs file is shared with the rest of the app now, so reading and
// writing it lives in lib/prefs.js. Only the update-related defaults are here.
function readPrefs() {
  return { ...DEFAULTS, ...store.read() };
}

function writePrefs(patch) {
  return { ...DEFAULTS, ...store.write(patch) };
}
