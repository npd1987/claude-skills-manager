...
function installKind(dir) {
  ...
  if (/[\\/]_npx[\\/]/.test(target)) {
    return {
      kind: 'npx',
      advice: 'This is one of npm\'s cache folders, so deleting it is safe — npm rebuilds it if you ever run the published version again.',
      command: null,
    };
  }
  ...
  return {
    kind: 'folder',
    advice: 'It\'s an ordinary folder — delete it whenever you like.',
    command: null,
  };
}
