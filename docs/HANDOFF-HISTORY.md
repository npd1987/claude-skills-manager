# Handoff history: superseded state of play, newest first

**This is the archive behind [HANDOFF.md](HANDOFF.md), and nothing here is current.** Each block was
the whole state of play at the end of the session that wrote it, and was moved here when the next
session wrote its own.

**THIS FILE IS A ROLLING WINDOW OF THE LAST THREE BLOCKS, AND IT IS PRUNED EVERY HANDOFF.** Nothing
reads it whole; it is grepped, and a grep costs in proportion to how many lines match, so an append
only archive gets dearer every session for the same question. The prune is committed on its own, so
the complete pre prune file is a permanent git object at that commit's parent. Anything durable in a
pruned block was supposed to have been written into a feature document, [DECISIONS.md](DECISIONS.md)
or [LESSONS.md](LESSONS.md) by the handoff that archived it; if a prune loses something, that step is
what failed.

---

# Block: 2026-09-10, the state of play from 1.1.2 until the reveal regression was found

**Written by the handoff at the end of the session that shipped 1.1.2, and superseded the same day
by the session that found the Folder button broken.** Its section 0 pointed at OI-2 as the next
thing, and that is still where the queue points; what interrupted it was a regression that round had
itself shipped. Kept verbatim.
...
# Block: 2026-09-10, the launch, window and maximize round

**Written by the handoff on 2026-09-10 and superseded the same day by the block that replaced it.**
It was the state of play from the moment 1.1.2 was released until the sweep that followed. Kept
verbatim.
...
# Block: 2026-08-01, the handoff that predates the method

**This is the project's own handoff document, kept verbatim, as it stood when the
`handoff-method` skill was set up on 2026-09-10.** It is evidence, not instruction. Nothing in it was
deleted; every part of it was moved into the method's files, and this is the map:

| What it held | Where it now lives |
| :--- | :--- |
| *Decisions that still bind*, all of them | [DECISIONS.md](DECISIONS.md) |
| *Traps*, all of them | [LESSONS.md](LESSONS.md), one entry each |
| *Live threads*, all of them | [OPEN-ITEMS.md](OPEN-ITEMS.md), as OI-2 through OI-8 |
| *How the user works* | [WORKING-WITH-THE-USER.md](WORKING-WITH-THE-USER.md) |
| *Verified, and not* | [HANDOFF.md](HANDOFF.md) section 8, and the table below is the record it was built from |
| *Where it stands* | [HANDOFF.md](HANDOFF.md) sections 0 and 1 |
| *Keeping this file honest*, the seven numbered rules | [HANDOFF-METHOD.md](HANDOFF-METHOD.md), the profile and its closing section |
| *Session log* | Below, kept as the record of what shipped when |

**The verification table below is still the best record of what has and has not been proved**, and
its version stamps are still accurate. Nothing in this round changed any row in it.
...
- **Renaming a skill** is designed but unbuilt. The design, the evidence for why
  it needs a reference scan rather than a find and replace, and the two open
  questions are in [RENAME-SKILL.md](RENAME-SKILL.md).
...
## Index of older blocks

None yet. The window has not been pruned.
